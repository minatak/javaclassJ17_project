...
public class MemberDAO {
	...
	public ArrayList<MemberVO> getMemberList() {
		...
		try {
			sql = "select * from member where userDel = 'NO' order by idx desc";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	public ArrayList<MemberVO> getMemberListLearningLanguage(String nativeLanguage, int startIndexNo, int pageSize) {
		ArrayList<MemberVO> vos = new ArrayList<MemberVO>();
		try {
			// sql = "select * from member where nativeLanguage=? union select * from member where nativeLanguage != ? and userDel = 'NO' limit ?,? order by idx desc";
			sql = "SELECT * FROM member WHERE userDel = 'NO' ORDER BY CASE WHEN nativeLanguage = ? THEN 0 ELSE 1 END, idx DESC LIMIT ?, ?";
